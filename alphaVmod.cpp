...
string s = "2ele1z/ppppppp/7/7/7/PPPPPPP/2ELE1Z w 1";
string nextS = "2ele1z/ppppppp/7/7/7/PPPPPPP/2ELE1Z w 1";
...
void calcAttackScore(pair<int, int> newMove){
    if(tempTurn=='b'){
        if(tempBoard[newMove.first][newMove.second]!='#'){
            blackAttackScore++;
            if(tempBoard[newMove.first][newMove.second]=='L'){
                blackAttackScore += 10;
            }
        }
    }
    ...
    for(int i=0; i<lionPossibleMoves.size(); i++){
        ...
        if(lionIsInBounds(newMove)){
            if(!isTeamMate(pos, newMove)){
                ...
                calcAttackScore(newMove);
            }
        }
    }
    if(res.size()>2){
        pair<int, int> pos2 = convertCoordinates(otherLion);
        if(otherLionIsOpen()){
            moves.push_back(coord+otherLion);
            calcAttackScore(pos2);
        }
        else if(isDiagonal()){
            moves.push_back(coord+otherLion);
            calcAttackScore(pos2);
...
int alphaBeta2(string currentState, int depth, int alpha, int beta){
    tempPositions = settingUpBoard(currentState);
    vector<string> res = splitString(currentState, ' ');
    tempBoard = putIntoBoard(res[0]);
    tempTurn = res[1][0];
    ...
    for(int i=0; i<moves.size(); i++){
        ...
        string move = moves[i];
        string nextState = makeMove(move);
        int eval = -alphaBeta2(nextState, depth-1, -beta, -alpha);
        if(eval >= beta){
            return beta;
        }
        if(eval > alpha){
...
int main(){
    while(true){
        ...
        if(sArr[1][0]=='b'){
            value = -1000000000;
            // iterative deepening
            for(int deepness = 2; deepness < 5; deepness++) {
                int newVal = alphaBeta2(s, deepness, -10000000, 10000000);
                value = max(value, newVal);
            }
            // value = alphaBeta(s, 4, -10000000, 10000000);
            s = nextS + 'w' + to_string(stoi(sArr[2])+1);
        }
        else{
            value = alphaBeta(s, 4, -10000000, 10000000);
            s = nextS + 'b' + sArr[2];
    ...
    s = "2ele1z/ppppppp/7/7/7/PPPPPPP/2ELE1Z b 1";
    while(true){
        // mini as black, alpha as white
        ...
        if(sArr[1][0]=='w'){
            ...
            s = nextS + 'b' + to_string(stoi(sArr[2])+1);
        }
        else{
            value = alphaBeta(s, 4, -10000000, 10000000);
            s = nextS + 'b' + sArr[2];
